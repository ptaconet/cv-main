...
cite_presentation <- function(ref_info) {
  ...
  if (length(dates) == 1) {
    ...
    #                          "{lubridate::day(dates)}")
    print_date <- glue::glue("{lubridate::day(dates)} ",
                             "{lubridate::month(dates, label = TRUE, abbr = FALSE)} ",
    ...
    if (lubridate::month(dates[1]) == lubridate::month(dates[2])) {
      ...
      print_date <- glue::glue("{lubridate::day(dates[1])}--{lubridate::day(dates[2])} ",
                               "{lubridate::month(dates[1], label = TRUE, abbr = FALSE)} ",
...
cite_report <- function(ref_info) {
  ...
  if (!is.na(cite_info$doi)) {
    ...
    cite <- glue::glue("{cite} [<a href={cite_info$doi}>PDF</a>]")
...
format_bib <- function(bib, emphasize = "Taconet, P.") {
